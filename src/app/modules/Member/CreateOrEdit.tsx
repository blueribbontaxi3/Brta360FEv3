...
const MemberCreateOrEditPage = (props: any) => {
    ...
    if (id) {
      const fetchMemberData = async () => {
        ...
        try {
          ...
          if (memberData) {
            setData(memberData.member)
            Object.keys(memberData.member).forEach((key) => {
              if (key !== 'dlIssuedDate' && key !== 'dlExpiryDate') {
                setValue(key, memberData.member[key]);
              } else if (key === 'dlIssuedDate') {
                setValue(
                  'dlIssuedDate',
                  [memberData.member.dlIssuedDate, memberData.member.dlExpirationDate]
                );
              }
            });
            setValue('status', memberData.member.status.toLowerCase() === 'active');
          }
        } catch (error: any) {
          if (error.status == 403) {
            navigate('/403')
  ...
  const breadCrumbList: any = [
    ...
    {
      title: (<Link to="/members">Member</Link>),
    },
    {
  ...
  const onSubmit: SubmitHandler<any> = async (data: any) => {
    ...
    console.log('-------------', data)
    if (id) {
      axios.post(`member/${id}`, data).then((r) => {
        setLoading(false)
        if (r.data.status === 1) {
      ...
      axios.post('member/store', data).then((r) => {
        setTimeout(() => {
          setLoading(false)
        }, 1999)
        if (r?.data?.status === 1) {
  ...
  return (
    <>
      <Banner breadCrumb={breadCrumbList} title={id ? 'Edit Member' : "Create Member"} data={data} />
...
export default MemberCreateOrEditPage;
