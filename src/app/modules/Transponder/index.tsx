...
import { Avatar, Button, Card, Col, notification, Popconfirm, Select, Space, Table, Tag, Typography } from 'antd';
...
const Transponder = () => {
  ...
  const columns = [
    {
      title: 'Transponder Number',
      dataIndex: 'transponderNumber',
      key: 'transponderNumber',
      render: (_: any, record: any) => record.transponderNumber,
    },
    {
      title: 'Medallion Number',
      dataIndex: 'medallionNumber',
      key: 'medallionNumber',
      render: (_: any, record: any) => record.medallionNumber,
    },
    {
      title: 'Apply Date',
      dataIndex: 'applyDate',
      key: 'applyDate',
      render: (_: any, record: any) => moment(record.applyDate).format('MM-DD-YYYY'),
    },
    {
      title: 'Created At',
      dataIndex: 'createdAt',
      ...
      render: (item: any, record: any) => {
        return (
          <Space direction="vertical">
            <Space wrap>
              {
                isPermission(authPermission, 'Transponder Update') &&
                <Link to={'/transponder/edit/' + record.id + ''}><Button type="primary" icon={<EditOutlined />} /></Link>
              }
              {
                isPermission(authPermission, 'Transponder Delete') &&
                <Popconfirm
                  title="Delete the affiliations"
                  description="Are you sure to delete this Transponders?"
                  okText="Yes"
                  cancelText="No"
  ...
  return (
    <>
      <Banner
        ...
        buttonTitle={'Create'}
        buttonUrl={'/transponder/create'}
        permission={'Transponder Create'}
      />
      <Card>
        <AdvanceTable
          data={data}
          ...
          onDelete={isPermission(authPermission, 'Transponders Delete') ? onDelete : ''}
        />
      </Card>
    </>
  );
